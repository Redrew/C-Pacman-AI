...
void ExitProgram(const char *message) {
    ...
    char *prop_mode = (propagation == max) ? "Max" : "Average";
    float search_time = (time(NULL) - time_start);

    fprintf(output, "-----------------------------------------\n");
    fprintf(output, "Propagation=%s\nBudget=%i\nMaxDepth = %i\n", prop_mode,
    budget, global_max_depth);
    fprintf(output, "TotalGenerated = %i\nTotalExpanded = %i\n", 
    total_generated, total_expanded);
    fprintf(output, "Time = %.2lf seconds\nExpanded/Second = %i\nScore = %i\n",
    search_time, (int) (total_expanded / search_time), Points);
    fprintf(output, "Level = %i\n", LevelNumber);
    fclose(output);
...
****************************************************************/
void MovePacman() {
    // If we are counting frames, increment frame
    if (frame_c != 0) frame_c++;
    ...
    switch (Level[Loc[4][0]][Loc[4][1]]) {
        ...
        case 3:    //PowerUp
            ...
            // Powerup, start counting frames
            frame_c = 1;
            break;
    }

    //Is he invincible?
    //The number of frames per game second is 1000 / SpeedOfGame, calculate time
    //left by frames
    if(Invincible == 1)  tleft = (11 - LevelNumber - 
        frame_c / (1000 / SpeedOfGame));

    //Is invincibility up yet?
    if(tleft < 0) { Invincible = 0; GhostsInARow = 0; tleft = 0; frame_c = 0; }
